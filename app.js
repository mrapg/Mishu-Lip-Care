...
// ── Free-Form 3D Fluid Fidget Interaction ──────────────────
function initFidgetSpinner() {
  ...
  console.log('🌀 Fluid 3D Fidget Controls active');
  ...
  mv.addEventListener('camera-change', (e) => {
    if (e.detail && e.detail.source === 'user-interaction') {
      ...
      // Sparkle trails on fluid spinning
      const now = Date.now();
      if (now - lastSparkleTime > 180) {
        lastSparkleTime = now;
        spawnTapParticle(['✨', '⭐', '🫧', '💧']);
      }
    }
  ...
  mv.addEventListener('pointerup', (e) => {
    ...
    // Quick tap (< 14px movement, < 320ms duration) -> Love Tap!
    if (dist < 14 && duration < 320) {
      handleJarTap(e);
    } else {
      // Light satisfying haptic tick on spin release
      if ('vibrate' in navigator) {
